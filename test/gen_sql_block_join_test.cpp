...
TEST(BLOCK_JOIN,GEN_GAP_SQL2) {
    int scale = 100000;
    fmt::print("create table t1 ( id float , t_name char (500));\ncreate table t2 (d_name char(500),id2 float);\n"); // 这里char填多一点，方便测试溢出的情况
    for(int i = 2; i <= scale; i+=2) {
        fmt::print("insert into t1 values({},'');\n",i);
    }
    for(int i = 3; i <= scale; i+=3) {
    ...
    fmt::print("select * from t1,t2 where t1.id  = t2.id2 order by t1.id DESC;\n");
